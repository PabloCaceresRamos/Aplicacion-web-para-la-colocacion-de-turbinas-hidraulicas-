    ...
    <body onload="inicializarCuenta()">
        ...
        <section class="p">
            ...
            <div class="ventana">
                ...
                <div>
                    <p class="mensaje"> Se ha enviado un correo para </p>
                    <p class="mensaje">el cambio de la contraseña</p>
                    <p class="mensajeTiempo" id="pSegundos"> Solicitar nuevo correo en:</p>
                    <input  id="temporizador"type="button" class="enviarCorreo" value="Enviar correo"/>
                    <form  action="/TurboWeb/Controlador/EnviarCorreoContrasena" method="POST">
                        <c:if test="${requestScope.correo!=null}"> <input id="correo" name="correo"  type="email" style="display:none;" value=${requestScope.correo} /></c:if>
                        <input  id="botonCorreo"type="submit" class="enviarCorreo" value="Enviar correo" />
                    </form>
                </div>
        ...
        <script>
            var fechaInicioCuenta;
            function inicializarCuenta() {
                fechaInicioCuenta = new Date();
                let minutos = fechaInicioCuenta.getMinutes() + 1;
                fechaInicioCuenta.setMinutes(minutos);//iniciamos la cuenta para un minuto
            }
            function cuentaAtras() {
                // codigo inspirado https://programadorwebvalencia.com/Javascript-cuenta-atras-o-contador-regresivo/
                let tiempoActual = new Date();
                let segundos = Math.floor((fechaInicioCuenta - tiempoActual) / 1000);
                let marcadorSegundos = document.getElementById("temporizador");
                marcadorSegundos.value = segundos + " Segundos";
                if (segundos === 0) {
                    marcadorSegundos.style.display = "none";
                    document.getElementById("botonCorreo").style.display = "block";
                }
            }

            setInterval(cuentaAtras, 1000);//cada segundo se actualiza
